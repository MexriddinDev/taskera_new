@extends('layouts.app')
@section('title', 'Ochiq Ticketlar - Taskera')
@section('content')
<div>
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-white">Ochiq Ticketlar</h1>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-700">
                ...
                <tbody class="divide-y divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-700/50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                            @if(strpos(strtolower($ticket->priority_name ?? ''), 'high') !== false || strpos(strtolower($ticket->priority_name ?? ''), 'crit') !== false)
                                <span class="text-red-400 font-medium">{{ $ticket->priority_name ?? '-' }}</span>
                            @else
                                {{ $ticket->priority_name ?? '-' }}
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <span class="px-2 py-1 text-xs rounded-full bg-blue-500/20 text-blue-400">{{ $ticket->status_name ?? 'Ochiq' }}</span>
                        </td>
                    ...
                    @empty
                    <tr>
                        <td colspan="8" class="px-6 py-4 text-center text-gray-500">Ochiq ticketlar topilmadi.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
