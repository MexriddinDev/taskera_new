@extends('layouts.app')
@section('title', (isset($isResolvedPage) && $isResolvedPage ? 'Hal qilingan ticketlar' : 'Barcha ticketlar') . ' - Taskera')
@section('content')
<div>
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-white">{{ isset($isResolvedPage) && $isResolvedPage ? 'Hal qilingan ticketlar' : 'Barcha ticketlar' }}</h1>
        <a href="/tickets/create" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition shadow-lg">+ Yangi Ticket</a>
    </div>
                ...
                <tbody class="divide-y divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-700/50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                            @if(strpos(strtolower($ticket->priority_name ?? ''), 'high') !== false || strpos(strtolower($ticket->priority_name ?? ''), 'crit') !== false)
                                <span class="text-red-400">{{ $ticket->priority_name ?? '-' }}</span>
                            @else
                                {{ $ticket->priority_name ?? '-' }}
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <span class="px-2 py-1 text-xs rounded-full bg-gray-600/50 text-gray-300">{{ $ticket->status_name ?? '-' }}</span>
                        </td>
                    ...
                    @empty
                    <tr>
                        <td colspan="8" class="px-6 py-4 text-center text-gray-500">Hech qanday ma'lumot topilmadi.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
